...
public class JedisCrud {
    ...
    static {
        ...
        //连接池资源用尽后调用者的最大等待时间，默认为-1
        cfg.setMaxWaitMillis(2000);
        //是否开启jmx监控
        cfg.setJmxEnabled(true);
        //连接的最小空闲时间，达到此值后空闲连接被移除
        cfg.setMinEvictableIdleTimeMillis(1000 * 60 * 30);
        //做空闲连接检测时，每次的采样数
        cfg.setNumTestsPerEvictionRun(3);
        //向连接池借用连接时是否做连接有效性检测(ping)，无效连接会被移除，每次借用多执行一次ping
        cfg.setTestOnBorrow(true);
        //向连接池归还连接时是否做连接有效性检测(ping)，无效连接会被移除，每次归还多执行一次ping
        cfg.setTestOnReturn(true);
        //向连接池借用连接时是否做连接空闲测试，空闲的会被移除
        cfg.setTestWhileIdle(true);
        //空闲连接的检测周期
        cfg.setTimeBetweenEvictionRunsMillis(-1);
        JedisPool jedisPool = new JedisPool(cfg, "117.48.203.244", 6385);
